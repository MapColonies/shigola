...
func NewLineFromSubPoints(points ...shigola.Point) (l Line) {
...
func (l Line) Subpoints() (points []shigola.Point) {
	points = make([]shigola.Point, 0, len(l))
	for i := range l {
		points = append(points, shigola.Point(l[i]))
	}
	return points
}
...
func (ml MultiLine) Lines() (lines []shigola.LineString) {
	lines = make([]shigola.LineString, 0, len(ml))
	for i := range ml {
		lines = append(lines, shigola.LineString(ml[i]))
	}
	return lines
}
