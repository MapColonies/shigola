...
The connection between shigola and PostGIS is configured in a `shigola.toml` file. An example minimum connection config:
...
In addition to the connection configuration above, Provider Layers need to be configured. A Provider Layer tells shigola how to query PostGIS for a certain layer. When using the PostGIS MVT Provider the `ST_AsMVTGeom()` MUST be used. An example minimum config using the `sql` config option:
