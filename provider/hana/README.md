# HANA
The HANA provider manages querying for tile requests against an [SAP HANA](https://www.sap.com/products/hana.html) database. The connection between shigola and HANA is configured in a `shigola.toml` file. An example minimum connection config:
...
## Provider Layers
In addition to the connection configuration above, Provider Layers need to be configured. A Provider Layer tells shigola how to query HANA for a certain layer. An example minimum config:
