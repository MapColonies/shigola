...
The connection between shigola and a GeoPackage is configured in a `shigola.toml` file. An example minimum connection config:
...
## Provider Layers
In addition to the connection configuration above, Provider Layers need to be configured. A Provider Layer tells shigola how to query a GeoPackage for a certain layer. An example minimum config:
