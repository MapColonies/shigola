...
azblob cache is an abstraction on top of Azure Blob Storage which implements the shigola cache interface. To use it, add the following minimum config to your shigola config file:
...
- `az_account_name` (string): [Optional] the storage account to use. `az_shared_key` must be specified with this property. If this is not set, Shigola will attempt an anonymous connection (requires a public blob conatiner) and treat the cache as read only.
...
- `read_only` (bool): [Optional] Shigola will not write cache missed tiles into the cache. This setting is implicitly set to `true` if no account credentials are given.
