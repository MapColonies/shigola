...
s3cache is an abstraction on top of Amazon Web Services (AWS) Simple Storage Service (S3) which implements the shigola cache interface. To use it, add the following minimum config to your shigola config file:

```toml
[cache]
type="s3"
bucket="shigola-test-data"
```
...
To make it work, the `req_signing_host` is a special parameter that forces Shigola to use a different HTTP Host header value when the AWS sdk signs the request to be sent to the real S3 endpoint. It needs to be set to the Host header (so no http:// prefixes etc..) of the real S3 endpoint (behind the reverse proxy for example).
