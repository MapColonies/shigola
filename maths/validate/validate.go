...
import (
	...
	"github.com/MapColonies/shigola/maths/clip"
	"github.com/MapColonies/shigola/maths/hitmap"
	"github.com/MapColonies/shigola/maths/makevalid"
...
func LineStringToSegments(l shigola.LineString) ([]maths.Line, error) {
	ppln := shigola.LineAsPointPairs(l)
	return maths.NewSegments(ppln)
}

func makePolygonValid(ctx context.Context, hm *hitmap.M, extent *geom.Extent, gs ...shigola.Polygon) (mp basic.MultiPolygon, err error) {
...
func scalePolygon(p shigola.Polygon, factor float64) (bp basic.Polygon) {
	lines := p.Sublines()
...
func scaleMultiPolygon(p shigola.MultiPolygon, factor float64) (bmp basic.MultiPolygon) {
	polygons := p.Polygons()
...
func CleanGeometry(ctx context.Context, g shigola.Geometry, extent *geom.Extent) (geo shigola.Geometry, err error) {
	if g == nil {
		return nil, nil
	}
	switch gg := g.(type) {
