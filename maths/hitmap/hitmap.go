...
func NewSegment(label maths.Label, linestring shigola.LineString) (seg Segment) {
...
func NewFromPolygon(p shigola.Polygon) (hm M) {
	sl := p.Sublines()
	if len(sl) == 0 {
		return hm
	}
...
func NewFromMultiPolygon(mp shigola.MultiPolygon) (hm M) {
	plgs := mp.Polygons()
	for i := range plgs {
...
func NewFromGeometry(g shigola.Geometry) (hm M) {
	switch gg := g.(type) {
