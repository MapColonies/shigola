...
func DrawPolygon(canvas *svg.SVG, p shigola.Polygon, id string, style string, pointStyle string) {
	var points []maths.Pt
	canvas.Gid(id)
	canvas.Gid("path")
	path := ""
...
func DrawLine(canvas *svg.SVG, l shigola.LineString, id string, style string, pointStyle string) {
	var points []maths.Pt
	pts := l.Subpoints()
...
func MinMaxForPolygon(p shigola.Polygon) (minx, miny, maxx, maxy int) {
	for _, l := range p.Sublines() {
...
func MinMaxForPolygonSlice(ps []shigola.Polygon) (minx, miny, maxx, maxy int) {
	for _, p := range ps {
