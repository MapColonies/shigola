...
func AreaOfPolygon(p shigola.Polygon) (area float64) {
	sublines := p.Sublines()
	if len(sublines) == 0 {
		return 0
	}
...
func AreaOfPolygonLineString(line shigola.LineString) (area float64) {
	// Only care about the outer ring.
...
func DistOfLine(l shigola.LineString) (dist float64) {
	points := l.Subpoints()
	if len(points) == 0 {
		return 0
	}
