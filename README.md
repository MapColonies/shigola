...
These three live in `TEGOLA_OPTIONS` rather than in `[cache]`, because they are
...
`sum(tegola_cache_tier_hits_total)` is **not** the chain hit count; use
...
| Metric | Means |
|---|---|
| `tegola_cache_write_slots_in_flight` / `_capacity` | pool saturation — the leading indicator |
...
> `errors`. It is now `tegola_cache_errors_total` (and
> `tegola_cache_tier_errors_total` per tier). Dashboards and alerts referring to
...
`rate(tegola_cache_tier_errors_total) > 0` should page, at least for the durable
...
Tegola is written in [Go](https://golang.org/) and requires [Go 1.26.2](https://go.dev/dl/) or higher to compile from the source.
