...
func TestTileDeg2Num(t *testing.T) {
	testcases := []struct {
		...
		{
			tile:      *shigola.NewTileLatLong(0, -85, -180),
			expectedX: 0,
			expectedY: 0,
		},
	}
...
func TestTileToFromPixel(t *testing.T) {
	tile := shigola.NewTile(20, 0, 0)
	fn := func(idx int, pt [2]float64) {
		npt, err := tile.FromPixel(shigola.WebMercator, pt)
		if err != nil {
			t.Errorf("[%v] Error, Expected nil Got %v", idx, err)
			return
		}
		gpt, err := tile.ToPixel(shigola.WebMercator, npt)
		if err != nil {
			t.Errorf("[%v] Error, Expected nil Got %v", idx, err)
			return
		}
