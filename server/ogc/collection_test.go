...
func TestTileCaching(t *testing.T) {
	...
	t.Run("the format selector shares one entry", func(t *testing.T) {
		...
		if got := get(t, r, tile+"?f=mvt", nil).Header().Get("Shigola-Cache"); got != "MISS" {
			t.Errorf("first request Shigola-Cache = %q, want MISS", got)
		}

		for _, f := range []string{"?f=mvt", "?f=pbf", "?f=PBF", ""} {
			if got := get(t, r, tile+f, nil).Header().Get("Shigola-Cache"); got != "HIT" {
				t.Errorf("%q Shigola-Cache = %q, want HIT", f, got)
			}
		}
	})
	...
	t.Run("a request carrying other parameters is not cached", func(t *testing.T) {
		...
		if got := get(t, r, tile+"?year=1990", nil).Header().Get("Shigola-Cache"); got != "" {
			t.Errorf("Shigola-Cache = %q, want no cache header at all", got)
		}

		// and it must not have populated the cache for everyone else
		if got := get(t, r, tile+"?f=mvt", nil).Header().Get("Shigola-Cache"); got != "MISS" {
			t.Errorf("after a parameterised request, Shigola-Cache = %q, want MISS", got)
		}
	})
