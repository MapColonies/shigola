...
func TestTileContent(t *testing.T) {
	...
	t.Run("the tile carries exactly these layers and features", func(t *testing.T) {
		...
		assertFeatureIDs(t, tile, "places", 1, 2, 3)
	...
	t.Run("attribute values are what the rows hold", func(t *testing.T) {
		assertTags(t, tile, "places", "centre", map[string]mvttest.Value{
			"name": mvttest.String("centre"), "rank": mvttest.Uint(10),
			"score": mvttest.Double(1.5), "active": mvttest.Bool(true),
	...
	t.Run("each layer declares its geometry type", func(t *testing.T) {
		for _, c := range []struct{ layer, want string }{
			{"places", "POINT"},
			{"roads", "LINESTRING"},
		} {
			l, ok := tile.Layer(c.layer)
			if !ok {
				t.Errorf("layer %q is missing", c.layer)
				continue
			}
			for _, f := range l.Features {
				if f.GeomType != c.want {
					t.Errorf("layer %q feature %d declares %v, want %v", c.layer, f.ID, f.GeomType, c.want)
				}
			}
		}
	})
	...
	t.Run("the encoding is negotiated", func(t *testing.T) {
		...
		for _, c := range []struct {
			name, accept, wantEncoding string
		}{
			{"advertising gzip", "gzip", "gzip"},
			{"advertising nothing", "", ""},
			{"refusing gzip", "gzip;q=0", ""},
		} {
			t.Run(c.name, func(t *testing.T) {
				resp := get(t, srv, uri, c.accept)

				if resp.Status != http.StatusOK {
					t.Fatalf("status = %d, want 200", resp.Status)
				}
				if resp.Encoding != c.wantEncoding {
					t.Errorf("Content-Encoding = %q, want %q", resp.Encoding, c.wantEncoding)
				}
				...
				t.Logf("  ok  Accept-Encoding %-9q -> Content-Encoding %q, %d bytes", c.accept, resp.Encoding, len(resp.Body))
			})
		}
	})
...
func TestTileContentBothSchemes(t *testing.T) {
	...
	t.Run("tags do not change with the scheme", func(t *testing.T) {
		...
		assertTags(t, mercator, "places", "centre", map[string]mvttest.Value{
			"name": mvttest.String("centre"), "rank": mvttest.Uint(10),
			"score": mvttest.Double(1.5), "active": mvttest.Bool(true),
			"note": mvttest.String("centre note"),
		})
	...
	t.Run("each scheme excludes ground the other serves", func(t *testing.T) {
		// The mercator tile stops at latitude 40.98; the geographic one reaches
		// 45. Ground between the two is served by one and not the other, which
		// is the latitude disagreement made visible. nulltag sits at 39.375, so
		// both hold it; a row above 40.98 would be held by only one.
		//
		// Rather than add a row, the same point is made with the tile below:
		// WebMercatorQuad 4/5/10 spans 40.98..55.77 and holds none of the
		// fixture, while the geographic tile covering that ground does.
		above := fetch(t, srv, contentCollection, tms.WebMercatorQuad, 4, 5, 10)
		absent(t, above, "places", "nulltag")

		here := fetch(t, srv, contentCollection, tms.WorldCRS84Quad, contentZoom, contentRow, contentCol)
		at(t, here, "places", "nulltag", point(3072, 1024))
	})
...
func TestTileContentAcrossZooms(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{zoom: 0, row: 0, col: 1, x: 1305, y: 1201},
		{zoom: 1, row: 0, col: 2, x: 2610, y: 2402},
		{zoom: 2, row: 1, col: 5, x: 1124, y: 708},
		{zoom: 3, row: 2, col: 10, x: 2248, y: 1416},
		{zoom: 4, row: 4, col: 21, x: 400, y: 2832},
		{zoom: 5, row: 9, col: 42, x: 800, y: 1568},
		{zoom: 6, row: 18, col: 84, x: 1600, y: 3136},
		{zoom: 7, row: 37, col: 168, x: 3200, y: 2176},
		{zoom: 8, row: 75, col: 337, x: 2304, y: 256},
	} {
		t.Run(fmt.Sprintf("zoom %d", c.zoom), func(t *testing.T) {
			tile := fetch(t, srv, contentCollection, tms.WorldCRS84Quad, c.zoom, c.row, c.col)
			at(t, tile, "places", "probe", point(c.x, c.y))

			// Exactly one tile per zoom holds it, and never on a boundary: the
			// neighbours on each side must not.
			for _, n := range []struct {
				label    string
				row, col int
			}{
				{"the column before", c.row, c.col - 1},
				{"the column after", c.row, c.col + 1},
			} {
				if n.col < 0 || n.col >= 1<<(c.zoom+1) {
					continue
				}
				absent(t, fetch(t, srv, contentCollection, tms.WorldCRS84Quad, c.zoom, n.row, n.col), "places", "probe")
			}
		})
	}
}
