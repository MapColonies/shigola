...
Shigola's built in viewer code is stored in the `ui/` directory. To build the ui `npm` must be installed. Once `npm` is installed the following command can be run from the repository root to generate a .go file for inclusion in the shigola binary:
...
The viewer can be excluded during building by using the build flag `noViewer`. For example, building shigola from the `cmd/shigola` directory:
