...
> and the error-variable form still apply. Its two-CGO-mode test rule does not: nothing in this fork
> is compiled conditionally on cgo, so there is one test mode.
>
...
* Work on the fix, and run all the tests. One mode is enough: nothing in this tree is compiled
  conditionally on cgo, so `CGO_ENABLED` no longer changes what is built or what is tested, and
  `internal/build` fails if that stops being true. Leave it unset — `go test -race` links a C
