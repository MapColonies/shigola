...
func TestParseErrorMessages(t *testing.T) {
	...
	tests := map[string]tcase{
		"bool from a number": {
			parse: func() error { _, err := env.ParseBool(int64(1)); return err },
		...
		"bool from a non-boolean string": {
			parse: func() error { _, err := env.ParseBool("xx"); return err },
		...
		"string from a number": {
			parse: func() error { _, err := env.ParseString(int64(1)); return err },
			want:  "1 (int64) is not a string",
		},
		"int from a float": {
			parse: func() error { _, err := env.ParseInt(1.5); return err },
			want:  "1.5 (float64) is not an integer",
		},
		"int from a non-integer string": {
			parse: func() error { _, err := env.ParseInt("xx"); return err },
			want:  `"xx" is not an integer`,
		},
		"uint from a negative number": {
			parse: func() error { _, err := env.ParseUint(int64(-1)); return err },
			want:  "-1 (int64) is not a non-negative integer",
		},
		"uint from a negative string": {
			parse: func() error { _, err := env.ParseUint("-1"); return err },
		...
		"float from an integer": {
			parse: func() error { _, err := env.ParseFloat(int64(1)); return err },
			want:  "1 (int64) is not a floating-point number (e.g. 1.0)",
		},
		"float from a non-numeric string": {
			parse: func() error { _, err := env.ParseFloat("xx"); return err },
			want:  `"xx" is not a floating-point number (e.g. 1.0)`,
		},
		"dict from a string": {
			parse: func() error { _, err := env.ParseDict("xx"); return err },
			want:  `"xx" is not a table`,
		},
		"url from a number": {
			parse: func() error { _, err := env.ParseURL(int64(1)); return err },
			want:  "1 (int64) is not a URL string",
		},
		"a missing env var still names the variable": {
			parse: func() error { _, err := env.ParseBool("${MAPCO_11617_UNSET}"); return err },
