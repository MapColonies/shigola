...
//
// Registering shigola_cache_hits_total once without a tier label and once with
...
func TestPerTierMetricsAreLabelled(t *testing.T) {
	...
	before := counter(t, "shigola_cache_tier_hits_total", map[string]string{"tier": "durable2", "sub_command": "get"})
	missBefore := counter(t, "shigola_cache_tier_misses_total", map[string]string{"tier": "hot2", "sub_command": "get"})
	chainBefore := counter(t, "shigola_cache_hits_total", map[string]string{"sub_command": "get"})

	if _, hit, err := a.GetCache().Get(context.Background(), obsKey); !hit || err != nil {
		t.Fatalf("get: got (%v, %v), expected (true, nil)", hit, err)
	}

	if got := counter(t, "shigola_cache_tier_hits_total", map[string]string{"tier": "durable2", "sub_command": "get"}); got != before+1 {
		t.Errorf("durable tier hits: got %v, expected %v", got, before+1)
	}
	if got := counter(t, "shigola_cache_tier_misses_total", map[string]string{"tier": "hot2", "sub_command": "get"}); got != missBefore+1 {
	...
	// summed together.
	if got := counter(t, "shigola_cache_hits_total", map[string]string{"sub_command": "get"}); got != chainBefore+1 {
...
func TestReadTimeoutIsCountedAsBoth(t *testing.T) {
	...
	errsBefore := counter(t, "shigola_cache_tier_errors_total", labels)
	timeoutsBefore := counter(t, "shigola_cache_tier_read_timeouts_total", labels)
	histBefore := counter(t, "shigola_cache_tier_duration_seconds", labels)

	if _, hit, err := a.GetCache().Get(context.Background(), obsKey); hit || err != nil {
		t.Fatalf("get: got (%v, %v), expected (false, nil) — the chain degrades to a miss", hit, err)
	}

	if got := counter(t, "shigola_cache_tier_errors_total", labels); got != errsBefore+1 {
		t.Errorf("errors: got %v, expected %v", got, errsBefore+1)
	}
	if got := counter(t, "shigola_cache_tier_read_timeouts_total", labels); got != timeoutsBefore+1 {
	...
	if got := counter(t, "shigola_cache_tier_duration_seconds", labels); got != histBefore+1 {
...
func TestClientDisconnectIsNotATierFault(t *testing.T) {
	...
	if got := counter(t, "shigola_cache_tier_errors_total", labels); got != errsBefore {
		t.Errorf("errors: got %v, expected it unchanged at %v", got, errsBefore)
	}
	if got := counter(t, "shigola_cache_tier_read_timeouts_total", labels); got != timeoutsBefore {
...
func TestDoubleSetObservability(t *testing.T) {
	...
	before := counter(t, "shigola_cache_tier_hits_total", labels)
	...
	if got := counter(t, "shigola_cache_tier_hits_total", labels); got != before+1 {
...
func TestNestedTierLabelsArePathQualified(t *testing.T) {
	...
	before := counter(t, "shigola_cache_tier_hits_total", map[string]string{"tier": "nested6/inner6b", "sub_command": "get"})

	if _, hit, err := a.GetCache().Get(context.Background(), obsKey); !hit || err != nil {
		t.Fatalf("get: got (%v, %v), expected (true, nil)", hit, err)
	}

	if got := counter(t, "shigola_cache_tier_hits_total", map[string]string{"tier": "nested6/inner6b", "sub_command": "get"}); got != before+1 {
	...
	if got := counter(t, "shigola_cache_tier_hits_total", map[string]string{"tier": "nested6", "sub_command": "get"}); got == 0 {
...
func TestPoolAndPromotionCollectors(t *testing.T) {
	...
	for _, name := range []string{
		"shigola_cache_write_slots_in_flight",
		"shigola_cache_write_slots_capacity",
		"shigola_cache_writes_dropped_total",
		"shigola_cache_writes_abandoned_total",
		"shigola_cache_writes_timed_out_total",
		"shigola_cache_writes_failed_total",
		"shigola_cache_writes_completed_total",
		"shigola_cache_write_duration_seconds_total",
		"shigola_cache_promotions_total",
		"shigola_cache_promotions_dropped_total",
	} {
		if !seen[name] {
	...
	if got := counter(t, "shigola_cache_write_slots_capacity", nil); got != 256 {
...
func TestErrorsCounterIsPrefixed(t *testing.T) {
	...
	if !seenFamily(t, "shigola_cache_tier_errors_total") {
		t.Error("shigola_cache_tier_errors_total is not published")
	}
