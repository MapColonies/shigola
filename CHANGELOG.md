...
> **This changelog is Tegola's.** Shigola is a fork of
> [go-spatial/tegola](https://github.com/go-spatial/tegola); every entry below records an upstream
> release and is kept verbatim as the project's history. Paths and command names in these entries
> are the ones that were correct at the time (`tegola`, `cmd/tegola_lambda`), not the ones this fork
> uses now. Changes made by the fork are described in [README.md](README.md#relationship-to-tegola).
