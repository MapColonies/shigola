...
Shigola's releases are below, newest first. Each entry is generated by
[release-please](https://github.com/googleapis/release-please) from the Conventional Commits merged
to the default branch, so a change appears here because its commit said what it was — see
[CONTRIBUTING.md](CONTRIBUTING.md#versioning-and-releases) for how a release is cut, and what the
first one breaks.

> **Everything from `0.17.0` down is Tegola's.** Shigola began as a fork of
> [go-spatial/tegola](https://github.com/go-spatial/tegola), and those entries are that project's
> release history, kept verbatim as this one's provenance. Paths and command names in them are the
> ones that were correct at the time (`tegola`, `cmd/tegola_lambda`), not the ones this fork uses
> now, and nothing regenerates them. What the fork changed is in
> [README.md](README.md#relationship-to-tegola).
